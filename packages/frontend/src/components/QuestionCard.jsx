  ...
  if (choices.length === 0 && !isGridIn) {
    return (
      <div className="bg-white rounded-2xl shadow-card overflow-hidden p-6">
        <p className="text-navy-500 text-center py-4">
    ...
    <div className="bg-white rounded-2xl shadow-card overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 border-b border-cream-200">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-navy-500">{question.domain}</span>
        </div>
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${
              showNoteEditor
                ...
                : 'text-navy-400 hover:text-gold-600 hover:bg-cream-100'
            }`}
            title="Add note"
          >
      ...
      <div className="p-6">
        {/* Question Text */}
        <div className="mb-6">
          <p className="text-navy-900 text-lg leading-relaxed font-medium">
            {question.question_text}
          </p>
          <p className="text-sm text-navy-500 mt-2">{question.skill}</p>
          {isGridIn && (
            ...
            <input
              ...
              className={`w-full p-4 text-lg rounded-xl border-2 transition-all ${
                showFeedback
                  ...
                  : 'border-cream-200 focus:border-gold-500 focus:outline-none'
              }`}
            />
        ...
        {showNoteEditor && (
          <div className="mt-4 p-4 bg-cream-100 rounded-xl">
            ...
            <textarea
              ...
              placeholder="Add your notes,解题思路, or reminders..."
              className="w-full p-3 rounded-lg border border-cream-300 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent text-navy-800 placeholder-navy-400 resize-none"
              rows={3}
            />
        ...
        {userNote && !showNoteEditor && userNote.content && (
          <div className="mt-4 p-3 bg-gold-50 rounded-xl border border-gold-200">
            <p className="text-navy-700 text-sm italic">"{userNote.content}"</p>
          </div>
        )}
      </div>
    </div>
